//
// Created by serboba on 04.02.22.
//
...
bool ompl::base::IsoManipulationOptimization::isCostBetterThan(const base::Cost &c1_, const base::Cost &c2_) const { // c1 is better than c2
    ...
    std::cout << "is cost better than "<< std::endl;
    std::cout <<"actions : " << c1_.action() << ", " << c2_.action() << " | dist : " << c1_.value() << ", " << c2_.value()  ;
    if(c1_.action()< c2_.action() || c1_.action()==1){
        std::cout << " - true" << std::endl;
        return true;
    }else if(c1_.action() == c2_.action()){
        if(c1_.value() < c2_.value()){
            std::cout << " - true" << std::endl;
            return true;
        }
    }
...
ompl::base::Cost ompl::base::IsoManipulationOptimization::motionCost(const State *s1, const State *s2) const {
    ...
    return Cost(si_->distance(s1,s2),action_cost);
}
